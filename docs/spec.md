...
> Write this to the GitHub issue tracker as a single issue (Repo issue tracker = GitHub; see `docs/agents/issue-tracker.md`). After creating it, apply the `ready-for-agent` triage label. `gh` was not available in the environment that produced this, so the spec is captured here verbatim — publish the body below as-is.
...
- **Catalog**: loaded from a CSV with columns `Name, Price, Quantity` (delimiter and edge-case handling for items without a quantity are parked TBC — see `docs/open-questions.md`). In-app additions and price fixes are allowed on top of the imported catalog. Items retain their identity after import; starting quantity is a one-time field for the stock check.
...
- **Vouchers — scope note**: vouchers are received only at this stall (never sold here), bought with cash elsewhere on/before the day. They're a fixed payment-tender type from the app's perspective for now; denomination and change policy (does a $50 voucher on a $37 sale give $13 change, spend in full, or carry forward?) are parked TBC in `docs/open-questions.md` and must be resolved before the voucher tender is finalized. The settlement rules above are written to allow the chosen policy without restructuring.
- **Export shape**: two CSVs per device — `sales.csv` with columns `device, sale_seq, created_at, updated_at, status, total, cash, octopus, voucher`, and `items.csv` with columns `device, sale_seq, item, quantity, price`. Voids appear in the same files with `status=voided` so the organizer can filter them out of totals but still see them. UTF-8, comma-delimited, with a header row.
...
- **Parked TBCs** (in `docs/open-questions.md`) that must be resolved before the affected slices are built: (1) voucher denominations & change policy — drives the final voucher-tender behavior; (2) voucher end-of-day handling — how collected vouchers are counted/handed back; (3) CSV offline transfer to the two devices — how the `Name, Price, Quantity` file physically reaches both laptops with no network (USB / pre-loaded / shared drive).
...
- **Build order hint** (not binding): catalog-load + setup → build-sale + settle → change + sequence number + write-per-sale → correction/void → cash adjustments + running totals → end-of-day view → CSV export → wipe-at-end. The parked TBCs gate the voucher-tender slices specifically, not the rest.